#!/bin/bash
#
# Load the latest (by date) clickstream data
#
...
# File system
TS_FILE=$MARKERS/clickstream-timestamp
...
echo "$program: Loading clickstream for $date" 1>&2

$ADMIN/clickstream-etl/clickstream-load-date.sh $date
status=$?
