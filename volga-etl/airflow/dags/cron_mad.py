...
if default_config:
    ...
    task_config = get_config('cron_mad.yaml')
else:
...
command = 'sudo env PATH=$PATH su -c "{acumen_cron_scripts_dir}/cron-mad.sh" {user}'.format(acumen_cron_scripts_dir=task_config['acumen_cron_scripts_dir'], user=task_config['user'])

t1 = BashOperator(task_id='cron_mad',
