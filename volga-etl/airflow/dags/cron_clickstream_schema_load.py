...
if default_config:
    ...
    task_config = get_config('cron_clickstream_schema_load.yaml')
else:
...
dag = DAG('cron_clickstream_schema_load',
...
command = 'sudo env PATH=$PATH su -c "{acumen_cron_scripts_dir}/cron-clickstream-schema-load.sh" {user}'.format(acumen_cron_scripts_dir=task_config['acumen_cron_scripts_dir'], user=task_config['user'])

t1 = BashOperator(task_id='cron_clickstream_schema_load',
