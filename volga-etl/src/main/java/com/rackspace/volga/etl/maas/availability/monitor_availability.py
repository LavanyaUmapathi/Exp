...
get_monitor_data="select a.check_id,a.check_monitoring_zone, a.start_time,a.end_time from (select check_id ,check_monitoring_zone, start_time, end_time from up_time_calculation.error_metrics_duration where (start_time >= {0} and start_time <{1}) or (start_time < {2} and end_time > {3}) UNION ALL select check_id ,check_monitoring_zone, start_time, 7227103875000 from up_time_calculation.open_errors where start_time < {4} ) a order by a.check_id,a.check_monitoring_zone, a.start_time"
get_suppressions = "select a.device, a.start_time, a.end_time from up_time_calculation.suppression_intervals a where (a.start_time < {0} and a.end_time > {1}) or (a.start_time >={2} and a.start_time  < {3})  order by a.device, a.start_time"
...
while now_time > start_date:
    ...
    df2=sqlContext.createDataFrame(monitoravailabilityRDD, ['monitor', 'monitoring_zone','downtime_duration','downtime_duration_with_suppression', 'up_time','up_time_percent', 'up_time_with_suppression_percent' ])
    df2.registerTempTable('monitor_up_time')
    insert_monitor_up_time="insert overwrite table up_time_calculation.monitor_monthly_availability partition (dt='{0}') select a.monitor,a.monitoring_zone, b.check_target, b.check_label, b.check_type, b.device_label,b.device_managed, b.device_uri,b.device, b.account ,a.downtime_duration,a.downtime_duration_with_suppression, a.up_time,a.up_time_percent, a.up_time_with_suppression_percent, b.source_system_name from monitor_up_time as a, up_time_calculation.monitor_details as b  where a.monitor=b.check_id".format(start_date.strftime('%Y-%m-%d'))
