...
def findoverlap(et):
    ...
    rset=[()]
    #print rcount
    if len(errorlist) == 1 and rcount==0:
	first_start=errorlist[0][0]
	first_end=errorlist[0][1]
    ...
    for i in range(0,len(errorlist)-1):
       ...
       if rcount == 0:
           ...
           if i == len(errorlist)-2:
                rset.append((monitor,second_start, second_end,second_end - second_start))
           continue
...
def calculateuptime(et):
    ...
    agreed_up_time=up_time-suppression_diff
    if agreed_up_time==0:
        #Aggreed time is 0 so avalibility is 1
        return(monitor, downtime_duration,downtime_with_suppression_duration, up_time,'{0:.6f}'.format(up_time_percent), '{0:.6f}'.format(1) )
    up_time_with_suppression_percent=float(1) - float(downtime_with_suppression_duration)/float(agreed_up_time)
...
load_monitors="insert overwrite table up_time_calculation.monitor_details  select check_id,check_target, check_label,"\
	      "check_type, device_label, device_managed, device_uri, device, account,source_system_name from up_time_calculation.vw_monitor_details "\
	 "UNION ALL select distinct monitor_id, check_target, check_label,check_type, device_label, device_managed, device_uri, "\
	" device, account,source_system_name "\
	" from up_time_calculation.vw_new_relic_monitor_details where dt>'{0}'".format(start_date.strftime('%Y-%m-%d'))
print load_monitors
...
get_data="select a.check_id, a.start_time,a.end_time,coalesce(b.rcount,1) as rcount from (select check_id , start_time, end_time from up_time_calculation.error_metrics_duration where "\
         "(start_time >= {0} and start_time <{1}) or (start_time < {2} and end_time > {3}) UNION ALL select check_id , start_time, 7227103875000 from up_time_calculation.open_errors "\
         " where start_time < {4}) a LEFT OUTER JOIN (select a.id, count(*) as rcount from (select id, substr(mz,3) as mzone from maas.checks lateral view explode(monitoring_zones_poll)"\
         " mzTable as mz) as a group by a.id UNION ALL select distinct monitor_id, 1 from new_relic.monitors where dt >= {5}) b"\
...
get_suppressions = "select a.device, a.start_time, a.end_time from up_time_calculation.suppression_intervals a where (a.start_time < {0} and a.end_time > {1}) or (a.start_time >={2} "\
         "and a.start_time  < {3})  order by a.device, a.start_time"
...
while now_time > start_date:
    ...
    print supression_end_ts
    print end_ts
    drop_partition_stmt="alter table  up_time_calculation.service_downtime_with_suppression drop if exists partition (dt='{0}');"\
            "alter table  up_time_calculation.service_monthly_availability drop if exists partition (dt='{1}');".format(start_date.strftime('%Y-%m-%d'),start_date.strftime('%Y-%m-%d'))
    ...
    current_statement = get_data.format(start_ts, end_ts, start_ts, start_ts, end_ts,start_date.strftime('%Y-%m-%d'))
    ...
    insert_stmt = "insert overwrite table up_time_calculation.service_downtime_with_suppression partition (dt='{0}') select a.monitor, b.check_target, b.check_label, b.check_type, "\
           "b.device_label,b.device_managed, b.device_uri,b.device, b.account ,a.start_time, a.end_time, a.duration , a.duration_with_suppression, b.source_system_name "\
           "from  dt_durations as a, up_time_calculation.monitor_details as b  where a.monitor=b.check_id".format(start_date.strftime('%Y-%m-%d'))
    ...
    if now_time.month == start_date.month:
    #    number_of_seconds_in_month=now_time.day*86400*1000+now_time.hour*3600*1000+now_time.minute*60*1000+now_time.second*1000
    ...
    insert_up_time_stmt = "insert overwrite table up_time_calculation.service_monthly_availability partition (dt='{0}') select a.monitor, b.check_target, b.check_label, b.check_type, "\
           "b.device_label,b.device_managed, b.device_uri,b.device, b.account ,a.downtime_duration,a.downtime_duration_with_suppression, a.up_time,a.up_time_percent, "\
           "a.up_time_with_suppression_percent, b.source_system_name from dt_up_time as a, up_time_calculation.monitor_details as b  "\
