...
echo $dateColumnName
echo $subject_orc
if [[ -e $AIRFLOW_RUN_DIR/$db_name/dates/$subject ]]
then
...
#if [[ $isDate == false ]]
#then
#   echo "Starting parmeter is not date , get max id from hive"
#   hiveGetMaxId="sudo -u hdfs $HIVE_HOME/bin/hive -S -e 'ADD JAR  hdfs:/hdp/apps/2.2.4.2-2/hive/auxjars/csv-serde-1.1.2-0.11.0-all.jar; select COALESCE(max($dateColumnName),0L)  from $db_name.$subject;' >  /tmp/_$subject.out"
#       echo $hiveGetMaxId
#       $dry eval $hiveGetMaxId  &
#       PID2=$!
#       echo "Hive get max id  PID is $PID2"
#       $dry wait $PID2
#       last_id = $(head -n 1  /tmp/_$subject.out)
#       echo $lasst_id
#
#fi
#exit
cdate=$(awk '{print $1}' $AIRFLOW_RUN_DIR/$db_name/dates/$subject)
cdatets=''
ndate=''
ndatets=''
if [[ $isDate == true ]]
...
then
        ...
        do
                ...
                echo $currDate > $AIRFLOW_RUN_DIR/$db_name/dates/$subject
                if [[ $orcFlag == "true" ]]
                then
                        if [[ $isTimestamp == true ]]
                        then
                                echo "insert table $db_name.$subject$orc select * from $db_name.$subject where $dateColumnName >= $cdatets and $dateColumnName < $ndatets;" >> /tmp/scripts/$subject
                        else
                                echo "insert table $db_name.$subject$orc select * from $db_name.$subject where $dateColumnName >= $cdate and $dateColumnName < $ndate;" >> /tmp/scripts/$subject
                        fi
                fi
       ...
       do
                ...
                echo  $last_id >  $AIRFLOW_RUN_DIR/$db_name/dates/$subject
        ...
        echo "Hive load orc finished"
        echo "completed"  >> /tmp/scripts/$subject
#        hiveGetMaxId="sudo -u hdfs $HIVE_HOME/bin/hive -S -e 'ADD JAR  hdfs:/hdp/apps/2.2.4.2-2/hive/auxjars/csv-serde-1.1.2-0.11.0-all.jar; select COALESCE(max($dateColumnName),0L)  from $db_name.$subject;' > $AIRFLOW_RUN_DIR/$db_name/ $subject.out"
#       echo $hiveGetMaxId
#       $dry eval $hiveGetMaxId  &
#       PID2=$!
#       echo "Hive get max id  PID is $PID2"
#       $dry wait $PID2
#       head -n 1 $AIRFLOW_RUN_DIR/$db_name/$subject.out > $AIRFLOW_RUN_DIR/$db_name/dates/$subject
