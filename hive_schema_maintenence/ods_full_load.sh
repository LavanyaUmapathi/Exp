...
then
    orc="_orc"
    hiveORCCmd="sudo -u hdfs $HIVE_HOME/bin/hive  -e 'ADD JAR  hdfs:/hdp/apps/2.2.4.2-2/hive/auxjars/csv-serde-1.1.2-0.11.0-all.jar;set hive.stats.autogather=false;set hive.support.quoted.identifiers=column; insert overwrite table $db_name.$subject$orc select * from $db_name.$subject;"
    echo $hiveORCCmd
    $dry eval $hiveORCCmd  &
    PID2=$!
