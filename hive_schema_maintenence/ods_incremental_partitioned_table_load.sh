  ...
  then
    ...
    then
        ...
        then
           ...
           echo $nextlastdate > dates/$subject
        fi
    else
...
if [[ $isTimestamp == true ]]
then
   sqoopcmd="sudo -u hdfs $SQOOP_HOME/bin/sqoop import --connect \"jdbc:sqlserver://$sql_ip;database=$db_name;username=$sql_u_name;password=$sql_p_word\"  --table $subject --where \"$dateColumnName>='$cdatets' and $dateColumnName<'$ndatets'\" --fields-terminated-by , --escaped-by \\\ --enclosed-by '\"'    --compress -m 1 --target-dir $newdir -- --schema $schema --table-hints NOLOCK"
else
   sqoopcmd="sudo -u hdfs $SQOOP_HOME/bin/sqoop import --connect \"jdbc:sqlserver://$sql_ip;database=$db_name;username=$sql_u_name;password=$sql_p_word\"  --table $subject --where \"$dateColumnName>='$cdate' and $dateColumnName<'$ndate'\" --fields-terminated-by , --escaped-by \\\ --enclosed-by '\"'    --compress -m 1 --target-dir $newdir -- --schema $schema --table-hints NOLOCK"
fi
echo $sqoopcmd
$dry eval $sqoopcmd
...
$dry wait $PID2
echo "Hive load partitions finished"
echo $cdate > $AIRFLOW_RUN_DIR/$db_name/dates/$subject
