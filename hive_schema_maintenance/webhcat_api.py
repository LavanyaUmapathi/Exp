...
def _get(url, accept=(200), params=[]):
...
def create_database(db_name, comment):
    if not database_exists(db_name):
        ...
        _put('ddl/database/' + db_name, json.dumps(db_json))
    return True
...
def create_table(db_name, table_name, comment, columns, partition_by, external_flag,
    ...
    _put('ddl/database/' + db_name + '/table/' + table_name, json.dumps(table_json))
...
def drop_table(db_name, table_name):
    url = cons.BASE_URL + 'ddl/database/' + db_name + '/table/' + table_name
    resp = requests.delete(url, params={'user.name': cons.WEBHCAT_USERNAME})
    if resp.status_code != 200:
