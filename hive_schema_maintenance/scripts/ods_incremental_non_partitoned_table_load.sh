...
then
        ...
        then
            ...
            then
                ...
                cmd="echo 'ORC table load failed for $subject. Please investigate' | mail -s 'ORC load failed for $subject ' Natasha.Gajic@rackspace.com"
                eval $cmd
                exit
            else
                ...
                hiveORCCmd="$HIVE_HOME/bin/hive  -f /tmp/scripts/$subject"
                echo $hiveORCCmd
                $dry eval $hiveORCCmd  &
                PID2=$!
...
then
        ...
        #$dry echo "ADD JAR  hdfs:/user/ods_archive/csv-serde-1.1.2-0.11.0-all.jar;" > /tmp/scripts/$subject
...
then
        ...
        do
                ...
                if [[ $isTimestamp == true* ]]
                then
                        sqoopcmd="$SQOOP_HOME/bin/sqoop import --connect \"jdbc:sqlserver://$sql_ip;database=$db_name;username=$sql_u_name;password=$sql_p_wo
rd\"  --table $subject --where \"$dateColumnName>=$cdatets and $dateColumnName<$ndatets\" --fields-terminated-by , --escaped-by \\\ --enclosed-by '\"'    --c
ompress -m 1 --target-dir $targetdir --append --hive-drop-import-delims -- --schema $schema --table-hints NOLOCK"
                else
                        sqoopcmd="$SQOOP_HOME/bin/sqoop import --connect \"jdbc:sqlserver://$sql_ip;database=$db_name;username=$sql_u_name;password=$sql_p_wo
rd\"  --table $subject --where \"$dateColumnName>='$cdate' and $dateColumnName<'$ndate'\" --fields-terminated-by , --escaped-by \\\ --enclosed-by '\"'    --c
ompress -m 1 --target-dir $targetdir --append --hive-drop-import-delims -- --schema $schema --table-hints NOLOCK"
                fi
                echo $sqoopcmd
                $dry eval $sqoopcmd
                ...
                then
                        if [[ $isTimestamp == true* ]]
                        then
                                echo "insert into table $db_name.$subject$orc select * from $db_name.$subject where cast($dateColumnName as bigint) >= $cdate
ts and cast($dateColumnName as bigint) < $ndatets;" >> /tmp/scripts/$subject
                        else
                                echo "insert into table $db_name.$subject$orc select * from $db_name.$subject where $dateColumnName >= '$cdate' and $dateColu
mnName < '$ndate';" >> /tmp/scripts/$subject
                        fi
                fi
       ...
       do
                ...
                sqoopcmd="$SQOOP_HOME/bin/sqoop import --connect \"jdbc:sqlserver://$sql_ip;database=$db_name;username=$sql_u_name;password=$sql_p_word\"  --
query 'select TOP 10000000 *  from $subject WITH (NOLOCK) where $dateColumnName > $cdate and \$CONDITIONS order by $dateColumnName asc' --fields-terminated-b
y , --escaped-by \\\ --enclosed-by '\"'    --compress -m 1 --target-dir $targetdir --check-column $dateColumnName --incremental append --last-value $cdate --
hive-drop-import-delims"
                ...
                then
                        ...
                        if [[ $orcFlag == "true" ]]
                        then
                                echo "insert into  table $db_name.$subject$orc select * from $db_name.$subject where cast($dateColumnName as bigint)  > $cdat
e and cast($dateColumnName as bigint) <= $last_id;" >> /tmp/scripts/$subject
                        fi
