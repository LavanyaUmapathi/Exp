...
def execute_re_run(ods_name, rerun_cmd):
    ...
    if (not os.path.exists(file_name) and (not os.path.exists(d_file_name))):
        rerun_cmd=rerun_cmd+con.AIRFLOW_JOBS_HOME+"/scripts/ods_archiving/checkDailyLoad.sh usldb_ods"
        print rerun_cmd
        f = open(file_name, 'w')
        f.write(rerun_cmd)
        f.close()
        ...
        for one_line in input_script:
            if not one_line.startswith("echo"):
                ...
                if  "full_" not in one_script:
                        (query_date,sql_server_r_count,rdd_r_count,orc_r_count)=oqc.check_table('usldb_ods',load_cmd[3]+'.'+load_cmd[1], 'True', load_cmd[2], (datetime.date.today()-datetime.timedelta(20)).strftime("%Y-%m-%d"))
