        ...
        for one_line in input_script:
            if not one_line.startswith("echo"):
                ...
                if  "rawxml" not in one_script:
                        (query_date,sql_server_r_count,rdd_r_count,orc_r_count)=oqc.check_table('cloud_usage_events',load_cmd[3]+'.'+load_cmd[1], 'True', load_cmd[2], (datetime.date.today()-datetime.timedelta(6)).strftime("%Y-%m-%d"))
                        ...
                        (query_date,sql_server_r_count,rdd_r_count,orc_r_count)=oqc.check_table('cloud_usage_events','rawxml.'+load_cmd[1], 'True', load_cmd[2], (datetime.date.today()-datetime.timedelta(6)).strftime("%Y-%m-%d"))
                        if (sql_server_r_count > rdd_r_count):
                                rerun_cmd=rerun_cmd+one_line
                                msg_text=msg_text+"Record counts do not match for database {} table {} and date {} : SQL Server Count {}, Hadoop Count {}\n".format('cloud_usage_events',load_cmd[1],query_date,sql_server_r_count,rdd_r_count)
