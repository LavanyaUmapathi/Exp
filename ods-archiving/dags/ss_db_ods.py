...
tv = BashOperator(task_id='verify_load',
                  bash_command=script_folder + 'ods_archiving/checkDailyLoad.sh ss_db_ods;',
                  dag=dag, 
                  trigger_rule=TriggerRule.ALL_DONE)

BashOperator(task_id='ods_load_batch_0',
             bash_command=script_folder + 'ss_db_ods/ods_load_batch_0.sh;',
             dag=dag).set_downstream(tv)

BashOperator(task_id='ods_load_batch_1',
             bash_command=script_folder + 'ss_db_ods/ods_load_batch_1.sh;',
             dag=dag).set_downstream(tv)

BashOperator(task_id='ods_load_batch_2',
             bash_command=script_folder + 'ss_db_ods/ods_load_batch_2.sh;',
             dag=dag).set_downstream(tv)

BashOperator(task_id='ods_load_batch_3',
             bash_command=script_folder + 'ss_db_ods/ods_load_batch_3.sh;',
             dag=dag).set_downstream(tv)

BashOperator(task_id='ods_load_batch_4',
             bash_command=script_folder + 'ss_db_ods/ods_load_batch_4.sh;',
             dag=dag).set_downstream(tv)
