#!/bin/bash
#
# Download MAAS metrics data from cloud files day by day
#
...
tmpdir="/home/maas/tmp"

out_file=$(mktemp)

# HDFS
HDFS_MAAS_METRICS_DIR="/user/maas/metric-data"
...
while test $date '<' $end_date -o $date '=' $end_date; do
    ...
    hdfs_date_dir="$HDFS_MAAS_METRICS_DIR/$date"

    now=`date +%Y-%m-%dT%H:%M:%S`
    echo "$program: $now Downloading from CF date $date" 1>&2
    date_nom=`echo $date | tr -d '-'`

    cmd="swift $SWIFT_ARGS download $SWIFT_DOWNLOAD_ARGS --prefix $date_nom metric-data"
    if test $dryrun = 1; then
    ...
    if test $nfiles -gt 0; then
        ...
        cmd="$HADOOP_FS -moveFromLocal * /user/maas/metric-data/$date/"
        if test $dryrun = 1; then
          echo "$program: DRYRUN would run $cmd"
        else
          $cmd
        ...
        cmd="$HADOOP_FS -du -s -h /user/maas/metric-data/$date/"
        if test $dryrun = 1; then
          echo "$program: DRYRUN would run $cmd"
        else
          $cmd
