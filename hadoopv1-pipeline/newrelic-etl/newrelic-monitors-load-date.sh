#!/bin/bash
#
# Load the New Relic monitors data for a given DATE
#
# USAGE: newrelic-monitors-load-date.sh [OPTIONS] YYYY-MM-DD [YYYY-MM-DD]
...
# File system
raw_data_file_list="/tmp/${program}-$$-monitors-data-files.lst"
...
HDFS_DATA_INPUT_DIR="$HDFS_NEWRELIC_INPUT_DIR-monitors"

NEWRELIC_RECORD_TYPE='monitor'

# Hive config
HIVE_DATABASE="$HIVE_NEWRELIC_DATABASE"
HIVE_TABLE="$HIVE_NEWRELIC_MONITORS_TABLE"
...
echo "$program: Running Map-Reduce job with $reducers reducers to load monitors" 1>&2
...
INSERT OVERWRITE TABLE ${HIVE_PROD_TABLE} PARTITION(dt)
  SELECT
    ...
    stg.location,
    stg.device_number,
    stg.device_source_system,
    stg.account_number,
    stg.account_source_system,
    stg.monitor_type,
    stg.monitor_description,
    '${end_date}' as DT
DISTRIBUTE BY DT
;
