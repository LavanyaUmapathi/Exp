...
print "In Cloud Events Usage DQ module"
shell_script_list=['/home/airflow/airflow-jobs/scripts/cloud_events_usage/backup_cloud_usage_events_with_orc1.sh',
'/home/airflow/airflow-jobs/scripts/cloud_events_usage/backup_cloud_usage_events_with_orc.sh',
'/home/airflow/airflow-jobs/scripts/cloud_events_usage/backup_glance_and_nova.sh',
'/home/airflow/airflow-jobs/scripts/cloud_events_usage/backup_cloud_usage_events_rawxml.sh',
'/home/airflow/airflow-jobs/scripts/cloud_events_usage/backup_cloud_usage_events_rawxml1.sh']
msg_text=""
rerun_cmd=""
    ...
    with open (one_script,'r') as input_script:
        for one_line in input_script:
            if not one_line.startswith("echo"):
                ...
                if  "rawxml" not in one_script:
                        (query_date,sql_server_r_count,rdd_r_count,orc_r_count)=oqc.check_table('cloud_usage_events',load_cmd[3]+'.'+load_cmd[1], 'True', load_cmd[2], (datetime.date.today()-datetime.timedelta(1)).strftime("%Y-%m-%d"))
                        ...
                        (query_date,sql_server_r_count,rdd_r_count,orc_r_count)=oqc.check_table('cloud_usage_events','rawxml.'+load_cmd[1], 'True', load_cmd[2], (datetime.date.today()-datetime.timedelta(1)).strftime("%Y-%m-%d"))
                        if (sql_server_r_count > rdd_r_count):
                                rerun_cmd=rerun_cmd+one_line
                                msg_text=msg_text+"Record counts do not match for database {} table {} and date {} : SQL Server Count {}, Hadoop Count {}\n".format('cloud_usage_events',load_cmd[1],query_date,sql_server_r_count,rdd_r_count)
...
if msg_text == "":
    ...
    subject="{} archiving successful".format('cloud_usage_events')
else:
    execute_re_run("cloud_usage_events", rerun_cmd)
    subject="{} archiving error".format('cloud_usage_events')
