...
if default_config:
    ...
    task_config = get_config('cron_hdfs_uploader_clickstream_schema.yaml')
else:
...
dag = DAG('cron_hdfs_uploader_clickstream_schema',
...
command = 'sudo env PATH=$PATH su -c "{acumen_cron_scripts_dir}/cron-hdfs-uploader-clickstream-schema.sh" {user}'.format(acumen_cron_scripts_dir=task_config['acumen_cron_scripts_dir'], user=task_config['user'])

t1 = BashOperator(task_id='cron_hdfs_uploader_clickstream_schema',
