package com.rackspace.foundation.informatica.cassandra.direct.loader.helper;
...
public class CassandraBulkLoader {
	...
	public void mainLoad(String args[]) throws IOException {
		LoaderOptions options = LoaderOptions.parseArgs(args);
		try {
			OutputHandler handler = new OutputHandler.SystemOutput(
					options.verbose, options.debug);
			SSTableLoader loader = new SSTableLoader(
					options.directory,
					new ExternalClient(handler, options.hosts, options.rpcPort),
					handler);
			DatabaseDescriptor
					.setStreamThroughputOutboundMegabitsPerSec(options.throttle);
			...
			if (options.noProgress) {
				...
				if (future.hadFailures()) {
					System.err
							.println("Streaming to the following hosts failed:");
	...
	static class ProgressIndicator {
		...
		public ProgressIndicator(
				Map<InetAddress, Collection<PendingFile>> filesByHost) {
			this.filesByHost = new HashMap<InetAddress, Collection<PendingFile>>(
					filesByHost);
		}
		...
		public boolean printProgress() {
			...
			long totalSize = 0;
			for (Map.Entry<InetAddress, Collection<PendingFile>> entry : filesByHost
					.entrySet()) {
				long progress = 0;
				long size = 0;
				int completed = 0;
				...
				sb.append(" (")
						.append(size == 0 ? 100L : progress * 100L / size)
						.append(")] ");
			}
			...
			sb.append("[total: ")
					.append(totalSize == 0 ? 100L : totalProgress * 100L
							/ totalSize).append(" - ");
			sb.append(mbPerSec(deltaProgress, deltaTime)).append("MB/s");
			sb.append(" (avg: ")
					.append(mbPerSec(totalProgress, time - startTime))
					.append("MB/s)]");
			;
	...
	static class ExternalClient extends SSTableLoader.Client {
		...
		public ExternalClient(OutputHandler outputHandler,
			...
			while (hostiter.hasNext()) {
				try {
					...
					Cassandra.Client client = createThriftClient(
							host.getHostAddress(), rpcPort);
					List<TokenRange> tokenRanges = client
							.describe_ring(keyspace);
					List<KsDef> ksDefs = client.describe_keyspaces();

					setPartitioner(client.describe_partitioner());
					@SuppressWarnings("rawtypes")
					Token.TokenFactory tkFactory = getPartitioner()
							.getTokenFactory();

					for (TokenRange tr : tokenRanges) {
						@SuppressWarnings("rawtypes")
						Range<Token> range = new Range<Token>(
					...
					if (!hostiter.hasNext())
						throw new RuntimeException(
								"Could not retrieve endpoint ranges: ", e);
				}
			}
		}
		...
		private static Cassandra.Client createThriftClient(String host, int port)
	...
	static class LoaderOptions {
		...
		public static LoaderOptions parseArgs(String cmdArgs[]) {
			...
			try {
				...
				if (cmd.hasOption(THROTTLE_MBITS))
					opts.throttle = Integer.valueOf(cmd
							.getOptionValue(THROTTLE_MBITS));

				if (cmd.hasOption(RPC_PORT_OPTION))
					opts.rpcPort = Integer.valueOf(cmd
							.getOptionValue(RPC_PORT_OPTION));

				if (cmd.hasOption(INITIAL_HOST_ADDRESS_OPTION)) {
					String[] nodes = cmd.getOptionValue(
							INITIAL_HOST_ADDRESS_OPTION).split(",");
					try {
		...
		private static CmdLineOptions getCmdLineOptions() {
			...
			options.addOption("p", RPC_PORT_OPTION, "rpc port",
					"port used for rpc (default 9160)");
		...
		public static void printUsage(Options options) {
			...
			new HelpFormatter()
					.printHelp(usage, header.toString(), options, "");
		}
	}
